...
-- Procedimientos
--
CREATE DEFINER = `admin` @`%` PROCEDURE `Asesinador` (IN `p_username` VARCHAR(255)) BEGIN DECLARE v_user_id INT;
...
SELECT
    CONCAT('No existe el usuario "', p_username, '".') AS Resultado;

ELSE -- Verificar si el usuario ya está marcado como muerto
IF EXISTS(
    SELECT
        1
    FROM
...
ELSE -- Marcar al usuario como muerto en la tabla de usuarios
UPDATE
    Users
SET
    dead = true
...
SELECT
    CONCAT('HA MUERTO "', v_user_name, '".') AS Resultado;
...
-- Funciones
--
CREATE DEFINER = `admin` @`%` FUNCTION `ObtenerIndividuoAleatorio` (`planeta_nombre` VARCHAR(255)) RETURNS VARCHAR(255) CHARSET utf8mb4 COLLATE utf8mb4_general_ci BEGIN DECLARE individuo_nombre VARCHAR(255);

SELECT
    username INTO individuo_nombre
FROM
    Users u
    JOIN Planetas p ON u.id_planeta_origen = p.id
WHERE
    p.nombre = planeta_nombre
ORDER BY
    RAND()
LIMIT
    1;
...
VALUES
    (
        1,
        'root',
        '2f8cc8e617a3b8b37a7875bbd04517082baf415ff8b82d308e67b8004e6925c2',
        '1971-06-28',
        'X1234567A',
        'white',
        0.00,
...
FROM
    (
        `Users` `u`
        join `Planetas` `pl` on(`u`.`id_planeta_origen` = `pl`.`id`)
    )
GROUP BY
    `pl`.`nombre`;
...
FROM
    (
        (
            `Planetas` `p`
            join `Users` `u` on(`p`.`id` = `u`.`id_planeta_origen`)
        )
